...
# Extract ID for all elk except 15
  ...
  cov_set <- as.character(cov_list[combo==g]$covs)
    ...
    for(i in unique(model_data$elk_id)){
      ...
      for(alg in alg_list){
        ...
        mod_output <- data.table(measure=c(mod_accuracy$V1, mod_specificity$V1, mod_sensitivity$V1), value=c(mod_accuracy$V2, mod_specificity$V2, mod_sensitivity$V2), elk_name=i, set=g)
...
### Save outputs ===
for(alg in alg_list){
  saveRDS(get(paste(alg, 'results', sep='_')), paste('results/', paste(nn_dist, paste('m_', paste(alg, 'results_blocked.rds', sep='_'), sep=''), sep=''), sep=''))
}
