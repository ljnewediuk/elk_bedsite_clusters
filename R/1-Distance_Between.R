#######################################################
#### 1 - Calculate distance between each cluster   ####
####     centre and point in the cluster, and      ####
####     determine how many points are within      ####
####     32 m of the cluster centre                ####
#######################################################
...
for(j in seq(1,100,1)){
  ...
  for(i in unique(clust_info$uid)){
    dat_sub <- dat[uid==i]
    # dat_sub <- dat_sub[3:7,] # Optionally subset clusters to 2.5 hours period
  ...
  no_se <- sd(clust_test[correct==0]$n_within_dist)/
    sqrt(length(clust_test[correct==0]$n_within_dist))
  yes_se <- sd(clust_test[correct==1]$n_within_dist)/
    sqrt(length(clust_test[correct==1]$n_within_dist))
...
for(clust in unique(clust_info$uid)){
  ...
  for(i in 1:nrow(dat_sub)){
    ...
    for(j in 1:nrow(not_pt)){
      ...
      pt_dist <- gdist(lat.1=dat_sub[i,]$lat, lon.1=dat_sub[i,]$lon, 
                       lat.2=not_pt[j,]$lat, lon.2=not_pt[j,]$lon, units="m")
